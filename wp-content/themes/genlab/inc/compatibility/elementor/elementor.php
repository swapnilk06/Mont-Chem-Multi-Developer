...
class Awaiken_Elementor { 
	...
	public static function register_new_widgets( $widgets_manager ) {
		...
		require_once GENLAB_THEME_DIR . '/inc/compatibility/elementor/widgets/site-logo.php';
		require_once GENLAB_THEME_DIR . '/inc/compatibility/elementor/widgets/casestudy.php';
		require_once GENLAB_THEME_DIR . '/inc/compatibility/elementor/widgets/template.php';
		require_once GENLAB_THEME_DIR . '/inc/compatibility/elementor/widgets/breadcrumb.php';

		$widgets_manager->register( new \genlab_widget_site_Logo() );
		$widgets_manager->register( new \genlab_widget_casestudy() );
		$widgets_manager->register( new \genlab_widget_template() );
		$widgets_manager->register( new \genlab_widget_breadcrumb() );

	}
	
	public static function elementor_editor_styles() {
		wp_enqueue_style( 'genlab-elementor-editor', GENLAB_THEME_URL . '/assets/css/elementor-editor.css', array(), GENLAB_THEME_VERSION );
		...
		if ( !defined( 'ELEMENTOR_PRO_VERSION' ) ) {
			...
			wp_add_inline_style( 'genlab-elementor-editor', $custom_css );
		}
	}
	
	public static function elementor_preview_styles() {
		wp_enqueue_style( 'genlab-elementor-preview-style', GENLAB_THEME_URL . '/assets/css/elementor-preview.css', array(), GENLAB_THEME_VERSION );
	}
	
	public function update_default_elementor_kit() {

		add_option( 'default_genlab_kit', 0 );
		if ( get_option( 'default_genlab_kit' ) === '0' ) {
			...
			update_option( 'default_genlab_kit', 1 );
		}
	}
