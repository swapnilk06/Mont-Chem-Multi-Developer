...
class Init {
	...
	public function enqueue_styles() {
		$screen = get_current_screen();
		if ( $screen->id == 'edit-elementskit_template' ) {
			wp_enqueue_style( 'select2', \ElementsKit_Lite::plugin_url() . 'assets/libs/select2/select2.min.css', [], \ElementsKit_Lite::version() );
	...
	public function enqueue_scripts() {
		$screen = get_current_screen();
		if ( $screen->id == 'edit-elementskit_template' ) {
			wp_enqueue_script( 'select2', \ElementsKit_Lite::plugin_url() . 'assets/libs/select2/select2.min.js', array( 'jquery' ), true, \ElementsKit_Lite::version() );
			...
			wp_localize_script(
				...
				'ekit_headerfooter_i18n',
				array(
					'all'    => esc_html__( 'All', 'elementskit-lite' ),
					'header' => esc_html__( 'Header', 'elementskit-lite' ),
					'footer' => esc_html__( 'Footer', 'elementskit-lite' ),
				)
			);
		}
	}
